...
public class MarkerFunctionalTest {
	...
	public void test() {
		String str ="a,{b|c},d";
		byte[] d = str.getBytes();
		Marker line = MarkerFactory.create(0, d.length-1);
		byte[] t1 = TokenFactory.create(",{");
		byte[] t2 = TokenFactory.create("{");
		byte[] t3 = TokenFactory.create("|");
		Marker m1 = line.splitAndGetMarker(d, t1,2);
		System.out.println(m1.toString(d));
		Marker m2 = m1.splitAndGetMarker(d, t2, 1);
		System.out.println(m2.toString(d));
		Marker m3 = m2.splitAndGetMarker(d, t3, 1);
